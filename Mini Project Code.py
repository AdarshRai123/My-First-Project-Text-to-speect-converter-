...
def read_fun(g):
    ...
    textnew = from_page.extractText()

    language = 'en'  
    obj = gTTS(text=textnew, lang=language, slow=False)   
    ...
    '''except:
        messagebox.showerror('Not Founded','File Not Found')'''
...
class tts(tkinter.Tk): 
    ...
    def __init__(self,*args,**kwargs):
        ...
        for F in (main_screen,main_menu,text2speech,pdf_reader):
            ...
            self.frames[F]=frame
            c_label=Label(self,text='Adarsh Rai - 10',bg='white',fg='blue',borderwidth=0,relief='solid').place(x=95,y=480)
            m_label=Label(self,text='Aadarsh Tiwari - 01',bg='white',fg='blue',borderwidth=0,relief='solid').place(x=95,y=500)
            n_label=Label(self,text='Arpit Singh - 42 ',bg='white',fg='blue',borderwidth=0,relief='solid').place(x=95,y=520)
...
class pdf_reader(Frame):
    def __init__(self,parent,controller):
        ...
        abc2btn=Button(self,text='->',command=lambda:read_fun(f),bg='white',borderwidth=1,relief='groove').place(x=180,y=140)
